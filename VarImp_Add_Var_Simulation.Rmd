...
n_toy <- 500
p_toy <- 3
err_toy <- rnorm(n_toy, mean = 0, sd = 0.05)
toy.data <- as.data.frame(matrix(rnorm(n_toy*p_toy, mean = 0, sd = 1), nrow = n_toy))
toy.data <- toy.data %>% 
  mutate(Y = 5*V1+5*V2+err_toy)
...
perm_add_var <- function(i, data.list, it, ntree3, replace){
  df <- data.list[[i]]
  new.df <- as.data.frame(replicate(n = it, expr = perm_rf(df = df, ntree3 = ntree3, replace = replace)))
...
df.list <- df_combs(toy.data)
rf.list <- each_pred_rf(df.list, ntree1 = 50)
rf.add.var.list <- rf_add_var(rf.list)
rf.add.imp.list <- rf_add_var_imp(rf.add.var.list, ntree2 = 50)
add.var.imp <- extract_var_imp(rf.add.imp.list)
rf.perm <- perm_rf(rf.add.var.list[[1]], ntree3 = 50, replace = TRUE)
perm.add.var <- perm_add_var(i = 1, data.list = rf.add.var.list, it = 5, ntree3 = 50, replace = TRUE)
rf.perm.add.var <- rf_perm_add_var(data.list = rf.add.var.list, it = 50, ntree3 = 50)
perm.add.var <- perm_add_var_randomforest(data = toy.data, it = 50, ntree1 = 50, ntree2 = 50, ntree3 = 50, replace = FALSE)
rf_plot_var_imp(perm.add.var)
rf_added_var_plot(perm.add.var)
perm.add.var[[1]]
perm.add.var[[3]]
...
```{r}
siml7.worep.results <- shallow_add_var_randomforest(data = siml.data2, ntree1 = 1000, 
                                                    ntree2 = 1000)
siml8.worep.results <- shallow_add_var_randomforest(data = siml.data2, ntree1 = 1000, 
                                                    ntree2 = 1000, replace = FALSE)
